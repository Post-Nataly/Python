...
class Chromosome:
    ...
    '''
    Теперь напишем функцию для генерации случайной хромосомы, которая принимает два параметра:
...
Напишем фразу для всей популяции, так как других применений у неё нет:
'''
    ...
    for chromo in population:
        ...
        for i in range(chromo.size):
            if chromo.genes[i] == final_chromo[i]:
                chromo.rating -= 1
'''
сделаем сортировку хромосом по рейтингу (обычная сортировка пузырьком):
'''
...
def repopulate(population, parents, children_count):
    ...
    которая заполняет втрорую половину популяции потомками (родители сохраняются в первой половине)
    '''
...
def get_parent_index(parents, exclude_index):
    ...
    от них потомков и восстановить втрорую половину популяции
...
def cross(chromo1, chromo2):
    ...
    Каждые два родителя порождают пару потомков. То есть функцию cross() мы вызываем дважды:
    сначала с (родитель1, родитель2), затем с (родитель2, родитель1).
...
его финальную строку (хромосому) мы закодируем в байтовые массивы
'''
gene_pool = bytearray(b'abcdefghijklmnopqrstuvwxyz ') #если хотите по-русски - ставите сюда весь русский алфавит
...
Список хромосом уже отсортирован по рейтингу и поэтому задача селекции решена (берём топ-10),
но ради будущей реализации других методов нам надо сделать формальный отбор. То есть - поместим
наших избранников в список "выживших".
